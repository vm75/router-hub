...
<head>
    ...
    :root {
      ...
      --asus-muted: #B0BEC5;
      --asus-tab: #2F3A3E;
      --asus-tab-active: #4D595D;
      --asus-btn-bg: linear-gradient(#233438 0%, #0F1011 100%);
      --asus-btn-hover: linear-gradient(#09639C 0%, #003047 100%);
      --green: #4cdb98;
      --red: #ff6b76;
      --amber: #ffcb66;
      --cyan: #31d7dc;
    ...
    /* ── Tab navigation: exact ASUS .tab / .tabClicked from index_style.css ── */
    /* These are redefined here for standalone; ASUS CSS wins in embedded mode */
    .rh-tabs {
      margin-bottom: -10px;
      max-width: 760px
    }
    ...
    .rh-tab {
      ...
      font: 12px/35px Verdana, "MS UI Gothic", "MS P Gothic", "Microsoft Yahei UI", sans-serif;
    ...
    /* ── ASUS form typography: exact matches from form_style.css ── */
    .rh-shell .formfonttitle {
      text-shadow: 1px 1px 0 #000;
      font: bold 16px/1.3 Arial, Helvetica, "MS UI Gothic", "MS P Gothic", "Microsoft Yahei UI", sans-serif;
      color: #fff;
      margin-left: 5px;
      margin-bottom: 10px
    }

    .rh-shell .formfontdesc {
      font: 13px/1.4 Arial, Helvetica, "MS UI Gothic", "MS P Gothic", "Microsoft Yahei UI", sans-serif;
      color: #fff;
      margin-left: 5px;
      margin-bottom: 10px
    }
    ...
    /* ── Title row (formfonttitle + right-side brand elements, Bandwidth Monitor pattern) ── */
    .rh-title-row {
      width: 100%;
    ...
    /* ── Cards (FormTable-inspired) ── */
    .card {
      grid-column: span 12;
      border: 1px solid var(--asus-border);
      margin-bottom: 12px;
      background: #2a3a42
    }
    ...
    .btn {
      ...
      color: #fff;
      height: 28px;
      font: 12px Verdana, "MS UI Gothic", sans-serif;
      padding: 0 10px;
      cursor: pointer;
      outline: none;
      white-space: nowrap
    ...
    .btn.primary {
      background: linear-gradient(#0a6ab5 0%, #003d6e 100%)
    }

    .btn.primary:hover {
      background: linear-gradient(#0e7dcf 0%, #004f90 100%)
    }

    .btn.danger {
      background: linear-gradient(#6b2030 0%, #3a0f1a 100%);
      color: #ffb8bf
    }
    ...
    .protection-setting .field {
      flex: 1;
      min-width: 0
    }

    .protection-setting .btn {
      flex-shrink: 0
    }
    ...
    .nginx-domain-row td {
      border-top: 1px solid #456170
    }

    .nginx-child-row td:first-child {
      padding-left: 28px;
      position: relative
    }

    .nginx-child-row td:first-child::before {
      color: var(--asus-muted);
      content: '└';
      left: 11px;
      position: absolute
    ...
    .nginx-root>summary::before {
      content: '+';
      color: var(--cyan);
      font-weight: bold
    }

    .nginx-root[open]>summary::before {
      content: '−'
    }

    .nginx-root>summary span:first-child {
      margin-right: auto
    }
    ...
    .nginx-accordion>summary::before {
      content: '+';
      color: var(--cyan);
      font-weight: bold
    }

    .nginx-accordion[open]>summary::before {
      content: '−'
    }

    .nginx-accordion>summary span:first-child {
      margin-right: auto
    }

    .nginx-accordion-body {
      padding: 0
    }
...
<body onload="initial();" class="bg">
  ...
  <div class="rh-shell">
    ...
    <div id="rh-nav" class="submenuBlock rh-tabs">
      ...
      <div class="tab rh-tab" data-view="services">Services</div>
      <div class="tab rh-tab" data-view="nginx">Nginx</div>
      <div class="tab rh-tab" data-view="certificates">Certificates</div>
      <div class="tab rh-tab" data-view="wol">Wake-on-LAN</div>
      <div class="tab rh-tab" data-view="firewall">Ban Shield</div>
      <div class="tab rh-tab" data-view="adguard">AdGuard</div>
    </div>

    <!-- Header row: formfonttitle left, brand/status right (AdaptiveQoS_Bandwidth_Monitor pattern) -->
    <div>&nbsp;</div>
    <table class="rh-title-row">
      <tr>
        <td class="title-cell formfonttitle">Router Hub &mdash; <span id="page-title">Dashboard</span></td>
        <td class="brand-cell">
          <div class="rh-brand-info">
            ...
            <span class="rh-version">v0.5.4</span>
    ...
    <div class="rh-tab-pane">
      <div class="formfontdesc" id="page-subtitle">Router status at a glance</div>
      ...
      <section class="view" id="view-services">
        <div class="card">
          <div class="card-head">
            <h2>Entware init services</h2>
            <div class="actions"><button class="btn" onclick="loadServices()">Refresh status</button></div>
          </div>
      ...
      <section class="view" id="view-adguard">
        <div class="card pad">
          <div class="toolbar">
            <div><strong>AdGuard Home</strong> <span id="adguard-status"></span></div>
            <div class="actions"><button class="btn primary" onclick="launchAdGuard()">Launch Web UI</button></div>
          </div>
        ...
        <div style="height:12px"></div>
        <div class="card pad">
          <h2 style="margin-top:0;font-size:13px">Integration Settings</h2>
          <form id="adguard-form" class="form" onsubmit="saveAdGuardSettings(event)">
            <div class="form-grid">
              ...
              <div class="field full">
                <label>LAN IP Address (used for DNS rewrites &amp; launch URL default)</label>
                <input class="input" name="lan_ip" placeholder="192.168.1.1">
              </div>
            </div>
      ...
      <section class="view" id="view-nginx">
        <div class="notice nginx-status-line"><span>Site changes are tested and reload a running nginx. Service
            lifecycle is managed from Services.</span><span id="nginx-status" class="muted">Loading…</span><div class="actions"><button class="btn" onclick="nginxCommand('test')">Test</button><button class="btn" onclick="nginxCommand('reload')">Reload</button></div></div>
        <details class="card nginx-card nginx-accordion">
          <summary><span>Sites</span><span class="muted">Domains, subdomains, and subfolders</span></summary>
          <div class="nginx-accordion-body">
            <div class="card-head">
              <h2>Manage sites</h2>
              <div class="actions"><button class="btn primary"
                  onclick="newNginxObject('domain')">+ Domain</button><button class="btn primary"
                  onclick="newNginxObject('subdomain')">+ Subdomain</button><button class="btn primary"
                  onclick="newNginxObject('subfolder')">+ Subfolder</button></div>
            </div>
            <div class="table-wrap">
              <table class="table">
                <thead>
                ...
                <tbody id="nginx-objects-body"></tbody>
              </table>
            </div>
          </div>
        </details>
        <details class="card nginx-card nginx-accordion">
          <summary><span>Templates</span><span class="muted">Reusable site configurations</span></summary>
          <div class="nginx-accordion-body">
            <div class="card-head">
              <h2>Configuration templates</h2>
              <div class="actions"><select class="input" id="template-kind" onchange="loadNginxTemplates()">
                  <option value="domain">Domain</option>
                  <option value="subdomain">Subdomain</option>
                  <option value="subfolder">Subfolder</option>
                </select><button class="btn primary" onclick="editNginxTemplate()">New</button></div>
            </div>
            <div id="nginx-templates-list"></div>
          </div>
        </details>
        <details class="card nginx-card nginx-accordion">
          <summary><span>Logs</span><span class="muted">Read-only, manually refreshed</span></summary>
          <div class="nginx-accordion-body">
            <div class="card-head">
              <h2>Nginx logs</h2>
              <div class="actions"><select class="input" id="nginx-log-select"></select><button class="btn"
                  onclick="viewNginxLog()">View / refresh</button></div>
            </div>
            <pre class="log" id="nginx-log-content">Select a log to view it. Logs do not auto-refresh.</pre>
          </div>
        </details>
        <details class="card nginx-root">
          <summary><span>Nginx root files</span><span class="muted">Infrequent, advanced configuration</span></summary>
          <div class="nginx-root-body">
            <div class="nginx-root-toolbar"><button class="btn primary" onclick="editNginxFile('')">New file</button>
            </div>
            <div id="nginx-files-list"></div>
          </div>
        </details>
      </section>

      <section class="view" id="view-certificates">
        <div class="notice warn">Certificates use the dehydrated script and hooks in the configured data directory.
        ...
        <div class="card">
          <div class="card-head">
            ...
            <div class="actions"><button class="btn" onclick="updateDehydrated()">Update dehydrated</button><button
                class="btn primary" onclick="editCertificate()">Add certificate</button></div>
          </div>
          <div class="table-wrap">
            <table class="table">
              <thead>
      ...
      <section class="view" id="view-wol">
        <div class="card">
          <div class="card-head">
            <h2>Machines</h2><button class="btn primary" onclick="editMachine()">Add machine</button>
          </div>
          <div class="table-wrap">
            <table class="table">
              <thead>
      ...
      <section class="view" id="view-firewall">
        <div class="notice" id="fw-health">Loading firewall health…</div>
        <div class="grid">
          <div class="card pad span-4">
            ...
            <div class="protection-setting">
              ...
              <button class="btn primary" onclick="saveFirewallToggle()">Save</button>
            </div>
            <hr style="border:0;border-top:1px solid #2a4050;margin:14px 0">
            <h3 style="font-size:13px">Allowlist</h3>
            <div class="actions"><input class="input" id="allow-network" placeholder="192.168.1.0/24"><button
                class="btn" onclick="addAllowlist()">Add</button></div>
            <div id="allow-list" style="margin-top:8px"></div>
            <hr style="border:0;border-top:1px solid #2a4050;margin:14px 0">
            <h3 style="font-size:13px">Quick actions</h3>
            <div class="protection-actions">
              <button class="btn" onclick="showActiveBans()">Active bans</button>
              <div class="manual-ban"><input class="input" id="manual-ban" placeholder="203.0.113.4/32"><button
                  class="btn danger" onclick="manualBan()">Ban 1 hour</button></div>
            </div>
          </div>
          <div class="card span-8">
            <div class="card-head">
              <h2>Detection rules</h2><button class="btn primary" onclick="editRule()">Add rule</button>
            </div>
            <div class="table-wrap">
              <table class="table">
                <thead>
  ...
  <div class="rh-overlay">
    <dialog class="modal" id="editor">
      <div class="modal-head">
        <h3 id="editor-title">Edit</h3><button class="btn small" onclick="closeEditor()">✕</button>
      </div>
    ...
    <dialog class="modal" id="log-modal">
      <div class="modal-head">
        <h3 id="log-title">Output</h3><button class="btn small"
          onclick="document.getElementById('log-modal').close()">✕</button>
      </div>
    ...
    <dialog class="modal bans-modal" id="bans-modal">
      <div class="modal-head">
        <h3>Active bans</h3><button class="btn small" onclick="closeActiveBans()">✕</button>
      </div>
      <div class="modal-body">
  ...
  <script>
    ...
    const CONFIG = { version: "0.5.4", apiBase: "{PROTOCOL}//{HOST}:3030", token: "router-hub-test-token" || sessionStorage.getItem('router-hub-token') || cookieToken };
    ...
    let certPollingTimer;
    let adguardPollingTimer;

    function stopCertificatePolling() { clearInterval(certPollingTimer); certPollingTimer = undefined; }
    function startCertificatePolling() { stopCertificatePolling(); certPollingTimer = setInterval(() => { if (state.view === 'certificates') loadCertificateLock(); }, 15000); }

    function stopAdguardPolling() { clearInterval(adguardPollingTimer); adguardPollingTimer = undefined; }
    function startAdguardPolling() { stopAdguardPolling(); adguardPollingTimer = setInterval(() => { if (state.view === 'adguard') loadAdguardStatusOnly(); }, 15000); }
    ...
    function showOutput(title, value) { const output = document.getElementById('log-content'); document.getElementById('log-title').textContent = title; output.textContent = typeof value === 'string' ? value : JSON.stringify(value, null, 2); document.getElementById('log-modal').showModal(); requestAnimationFrame(() => { output.scrollTop = output.scrollHeight }) }
    ...
    // keep canonical markup and restore our tabs if the firmware replaces them.
    const RH_TAB_MARKUP = document.getElementById('rh-nav')?.innerHTML || '';
    const RH_TAB_COUNT = Object.keys(titles).length;
    let rhTabObserver;
    function rhInstallAsusTabs(asusTabs) {
      asusTabs.innerHTML = RH_TAB_MARKUP;
      asusTabs.classList.add('rh-tabs');
    ...
    function rhSetupTabs() {
      const asusTabs = document.getElementById('tabMenu');
      const ownNav = document.getElementById('rh-nav');
      if (asusTabs && ownNav) {
        rhInstallAsusTabs(asusTabs);
        ownNav.style.display = 'none';
        if (!rhTabObserver) {
          rhTabObserver = new MutationObserver(() => {
            if (asusTabs.querySelectorAll('.rh-tab').length !== RH_TAB_COUNT) rhInstallAsusTabs(asusTabs);
          });
          rhTabObserver.observe(asusTabs, { childList: true });
        }
      }
      const model = document.getElementById('router-model');
      if (model && typeof based_modelid === 'string' && based_modelid.trim()) model.textContent = based_modelid;
      const navRoot = asusTabs || ownNav;
      if (!navRoot) return;
      if (!navRoot.dataset.rhTabsBound) {
        navRoot.dataset.rhTabsBound = 'true';
        navRoot.addEventListener('click', event => {
          const tab = event.target.closest('[data-view]'); if (!tab) return;
          navRoot.querySelectorAll('[data-view]').forEach(x => {
            x.classList.toggle('tabClicked', x === tab);
          ...
          if (previousView === 'certificates' && state.view !== 'certificates') stopCertificatePolling();
          if (previousView === 'adguard' && state.view !== 'adguard') stopAdguardPolling();
    ...
    if (typeof initial === 'function') {
      const _origInitial = initial;
      window.initial = function () { try { return _origInitial() } finally { rhSetupTabs() } };
    }
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', rhSetupTabs);
    } else {
      rhSetupTabs();
    }
    ...
    function nginxPath(value) { return value.split('/').map(encodeURIComponent).join('/') }
    ...
    function nginxObjectRow(o) { const child = o.kind !== 'domain', kind = esc(o.kind), domain = esc(o.domain), name = esc(o.name); return `<tr class="${child ? 'nginx-child-row' : 'nginx-domain-row'}"><td><strong>${esc(o.display_name || o.name)}</strong> <span class="tag">${kind}</span></td><td>${o.port ?? '<span class="muted">—</span>'}</td><td>${o.template ? `<span class="tag">${esc(o.template)}</span>` : '<span class="muted">Custom</span>'}</td><td>${nginxObjectStatus(o)}</td><td><div class="row-actions"><button class="btn small" onclick="editNginxObject('${kind}','${domain}','${name}')">Edit</button><button class="btn small" onclick="nginxObjectAction('${kind}','${domain}','${name}','${o.enabled ? 'disable' : 'enable'}')">${o.enabled ? 'Disable' : 'Enable'}</button><button class="btn small danger" onclick="deleteNginxObject('${kind}','${domain}','${name}')">Delete</button><button class="btn small action" title="Launch site" aria-label="Launch site" onclick="launchNginxSite('${kind}','${domain}','${name}')"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-label="Open URL"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg></button></div></td></tr>` }
    ...
    async function nginxCommand(command) { try { const r = await api(`/nginx/${command}`, { method: 'POST' }); nginxCommandResult(command, r); await Promise.all([loadNginxStatus(), loadNginxObjects()]) } catch (e) { const action = command === 'test' ? 'Nginx configuration test' : 'Nginx reload'; showOutput(action, `${action} failed: ${e.message}`) } }
    function nginxFileTree(files) { const root = { dirs: Object.create(null), files: [] }; files.forEach(file => { const parts = file.path.split('/'), name = parts.pop(); let node = root; parts.forEach(part => node = node.dirs[part] || (node.dirs[part] = { dirs: Object.create(null), files: [] })); node.files.push({ ...file, name }) }); const render = node => `${Object.keys(node.dirs).sort().map(name => `<details class="nginx-dir"><summary>${esc(name)}</summary>${render(node.dirs[name])}</details>`).join('')}<div class="nginx-files">${node.files.sort((a, b) => a.name.localeCompare(b.name)).map(file => `<div class="nginx-file"><span><strong>${esc(file.name)}</strong> <span class="nginx-file-meta">${file.size} bytes · ${prettyDate(file.modified)}</span></span><span class="row-actions"><button class="btn small" data-nginx-path="${esc(file.path)}" onclick="editNginxFile(this.dataset.nginxPath)">Edit</button><button class="btn small danger" data-nginx-path="${esc(file.path)}" onclick="deleteNginxFile(this.dataset.nginxPath)">Delete</button></span></div>`).join('')}</div>`; return `<div class="nginx-tree">${render(root)}</div>` }
    ...
    async function certificateAction(id, action) { try { const p = api(`/certificates/${id}/${action}`, { method: 'POST' }); setTimeout(loadCertificateLock, 500); const r = await p; showOutput(`Certificate ${action}`, r); loadCertificatesWithLock() } catch (e) { toast(e.message, true) } }
    ...
    async function loadAdguardStatusOnly() { if (!state.adguard?.enabled) return; try { const st = await api('/adguard/status'); state.adguardStatus = st; renderAdguardStatus(st) } catch (e) { document.getElementById('adguard-status').innerHTML = status(true, 'Enabled', 'Disabled') } }
    async function loadAdguard() { try { const cfg = await api('/adguard/config'); state.adguard = cfg; const f = document.getElementById('adguard-form'); if (f) { f.elements.enabled.checked = !!cfg.enabled; f.elements.api_endpoint.value = cfg.api_endpoint || ''; f.elements.launch_url.value = cfg.launch_url || ''; f.elements.username.value = cfg.username || ''; f.elements.password.value = ''; f.elements.lan_ip.value = cfg.lan_ip || '' } if (cfg.enabled) { await loadAdguardStatusOnly() } else { document.getElementById('adguard-status').innerHTML = status(false, 'Enabled', 'Disabled'); renderAdguardStatus(null) } if (state.view === 'adguard') startAdguardPolling(); } catch (e) { document.getElementById('adguard-status').textContent = 'API Error'; toast(e.message, true) } }
    ...
    async function toggleAdguardFiltering(enabled, duration_minutes) { try { const p = { enabled, duration_minutes: duration_minutes || null }; await api('/adguard/filtering', { method: 'POST', body: JSON.stringify(p) }); toast(`AdGuard filtering ${enabled ? 'enabled' : `paused for ${duration_minutes} min`}`); loadAdguardStatusOnly() } catch (e) { toast(e.message, true) } }
    async function toggleAdguardProtection(enabled, duration_minutes) { try { const p = { enabled, duration_minutes: duration_minutes || null }; await api('/adguard/protection', { method: 'POST', body: JSON.stringify(p) }); toast(`AdGuard protection ${enabled ? 'enabled' : `paused for ${duration_minutes} min`}`); loadAdguardStatusOnly() } catch (e) { toast(e.message, true) } }
    ...
    async function loadCertificatesWithLock() {
      ...
      if (state.view === 'certificates') startCertificatePolling();
    }
